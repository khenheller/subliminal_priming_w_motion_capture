...
while 1
    ...
    if ~isempty(markers(1))
        ...
        if ~isempty(lastPos)
                ...
                prev_x_pos = round(lastPos(1)/mmPerPixel); % in pixels.
                prev_y_pos = mainRect(4)-round(lastPos(2)/mmPerPixel);
                new_x_pos = round(curLocation(1)/mmPerPixel);
                new_y_pos = mainRect(4)-round(curLocation(2)/mmPerPixel);
                Screen('DrawLine', mainWin ,color, prev_x_pos, prev_y_pos, new_x_pos, new_y_pos,10);
