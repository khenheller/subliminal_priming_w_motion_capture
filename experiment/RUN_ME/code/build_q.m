...
function q = build_q(trials, name, p)
    ...
    switch name
        ...
        case 'categor_wo_prime'
            ...
            q.txtr(p.MASK1_TIME) = p.MASK1_TXTR;
            q.txtr(p.MASK2_TIME) = p.MASK2_TXTR;
            q.txtr(p.MASK3_TIME - p.PRIME_DURATION) = p.MASK3_TXTR;
            q.txtr(p.TARGET_TIME - p.PRIME_DURATION) = p.CATEGOR_TXTR;
            q.txtr(p.CATEGOR_TIME - p.PRIME_DURATION) = p.CATEGOR_TXTR;
            q.txtr(p.LATE_RES_TIME - p.PRIME_DURATION) = p.LATE_RES_TXTR;
            q.txt1(p.TARGET_TIME - p.PRIME_DURATION, :) = double(trials.target{1});
            q.name(p.FIX_TIME) = 'fix';
            q.name(p.MASK1_TIME) = 'mask1';
            q.name(p.MASK2_TIME) = 'mask2';
            q.name(p.MASK3_TIME - p.PRIME_DURATION) = 'mask3';
            q.name(p.TARGET_TIME - p.PRIME_DURATION) = 'target';
            q.name(p.CATEGOR_TIME - p.PRIME_DURATION) = 'categor';
            q.name(p.LATE_RES_TIME - p.PRIME_DURATION) = 'late_res';
        case 'recog'
