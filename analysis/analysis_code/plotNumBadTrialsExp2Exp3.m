...
function [] = plotNumBadTrialsExp2Exp3(traj_name, plt_p, p)
    ...
    for i_reason = 1:num_reasons
        beesdata{:, i_reason*2 - 1} = exp2_n_bad_trials{p.EXP_2_SUBS, i_reason}';
        beesdata{:, i_reason*2}     = exp3_n_bad_trials{p.EXP_3_SUBS, i_reason}';
    end
    yLabel = 'Number of bad trials';
    XTickLabel = [];
    colors = repmat({plt_p.reach_color, plt_p.keyboard_color},1,num_reasons);
    title_char = "Amount of bad trials comparison between Exp 2 & 3";
    hold on;
    % Plot.
    ...
    exp2_valid_trials(isnan(exp2_valid_trials)) = [];
    exp3_valid_trials(isnan(exp3_valid_trials)) = [];
    [~, p_val, ci, stats] = ttest2(exp3_valid_trials, exp2_valid_trials);
    disp('--------Avg num of Valid trials, Exp 2vs3 --------');
