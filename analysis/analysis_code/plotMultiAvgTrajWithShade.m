...
function [] = plotMultiAvgTrajWithShade(traj_names, plt_p, p)
    ...
    for iTraj = 1:length(traj_names)
        ...
        % Plot avg with shade.
        stdshade(avg_each.traj(iTraj).con_left(:,good_subs,1)', plt_p.f_alpha*0.3, plt_p.con_col, reach_subs_avg.traj.con_left(:,3), 0, 0, 'ci', plt_p.alpha_size, plt_p.linewidth);
        stdshade(avg_each.traj(iTraj).con_right(:,good_subs,1)', plt_p.f_alpha*0.3, plt_p.con_col, reach_subs_avg.traj.con_right(:,3), 0, 0, 'ci', plt_p.alpha_size, plt_p.linewidth);
        stdshade(avg_each.traj(iTraj).incon_left(:,good_subs,1)', plt_p.f_alpha*0.3, plt_p.incon_col, reach_subs_avg.traj.incon_left(:,3), 0, 0, 'ci', plt_p.alpha_size, plt_p.linewidth);
        stdshade(avg_each.traj(iTraj).incon_right(:,good_subs,1)', plt_p.f_alpha*0.3, plt_p.incon_col, reach_subs_avg.traj.incon_right(:,3), 0, 0, 'ci', plt_p.alpha_size, plt_p.linewidth);
        
        % Permutation testing.
        clusters = permCluster(avg_each.traj.con(:,good_subs,1), avg_each.traj.incon(:,good_subs,1), n_perm);

        % Plot clusters.
        points = [avg_each.traj.con(clusters.start, good_subs(1), 3)'; avg_each.traj.con(clusters.end, good_subs(1), 3)'];
        ...
        printTsStats('----Deviation From center--------', clusters.size, clusters.p_val, clusters.dz, clusters.t_star);
    end
end