...
function [h, u, y_lims, x_lims] = raincloud_plot(X, varargin)
...
% y/x_lims - lower and highest values on x/y axis. Used to fix lims outside this function.
%
% ------------------------ EXAMPLE USAGE -------------------------
%
...
xl = get(gca, 'XLim');
set(gca, 'XLim', [-xl(2)*lwr_bnd xl(2)]);
...
% raindrops
if box_dodge
    drops_pos = (jit * 0.6) - xl(2) * dot_dodge_amount;
else
    drops_pos = jit - xl(2) / 2;
end

h{2} = scatter(drops_pos, X);
h{2}.SizeData = 70;
...
if box_on
    ...
    if box_dodge
        box_pos = [((-xl(2) * box_dodge_amount) - (wdth * 0.3)), Y(1), (wdth * 0.6), Y(2) - Y(1)];
        
        % median line
        h{4} = line([((-xl(2) * box_dodge_amount) - (wdth * 0.3)) ((-xl(2) * box_dodge_amount) + (wdth * 0.3))], [Y(3) Y(3)], 'col', color, 'LineWidth', line_width+2);
        h{4} = line([((-xl(2) * box_dodge_amount) - (wdth * 0.3)) ((-xl(2) * box_dodge_amount) + (wdth * 0.3))], [Y(3) Y(3)], 'col', [0.7 0.7 0.7 0.7], 'LineWidth', line_width+2);
        
        % whiskers
        h{5} = line([(-xl(2) * box_dodge_amount) (-xl(2) * box_dodge_amount)], [Y(2) Y(5)], 'col', bxcl, 'LineWidth', line_width);
        h{6} = line([(-xl(2) * box_dodge_amount) (-xl(2) * box_dodge_amount)], [Y(1) Y(4)], 'col', bxcl, 'LineWidth', line_width);
    else
        box_pos = [-xl(2)/2-(wdth * 0.5), Y(1), wdth, Y(2)-Y(1)];
        % median line
        h{4} = line([-xl(2)/2-(wdth * 0.5) -xl(2) / 2 + (wdth * 0.5)], [Y(3) Y(3)], 'col', color, 'LineWidth', line_width+2);
        h{4} = line([-xl(2)/2-(wdth * 0.5) -xl(2) / 2 + (wdth * 0.5)], [Y(3) Y(3)], 'col', [0.7 0.7 0.7 0.7], 'LineWidth', line_width+2);
        
        % whiskers
        h{5} = line([-xl(2)/2 -xl(2)/2], [Y(2) Y(5)], 'col', bxcl, 'LineWidth', line_width);
        h{6} = line([-xl(2)/2 -xl(2)/2], [Y(1) Y(4)], 'col', bxcl, 'LineWidth', line_width);
    end
    % 'box' of 'boxplot'
    ...
    xlim('tickaligned') % set the ylim to capture all data
end

% Save to fix x,y lim otuside the function.
y_lims = [min(Xi), max(Xi)];
x_lims = [min(drops_pos), max(f)];