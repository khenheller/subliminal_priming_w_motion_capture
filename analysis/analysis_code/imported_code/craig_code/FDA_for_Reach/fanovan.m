function [p,corrP,t,stats,terms] = fanovan(y,group,varargin)
...
%
%   FANOVA edit
%   By Craig Chapman, February, 2011
%
%   All changes to original anovan function are preceded by the line:
%   FANOVA edit
%   ...with a brief description of the change made.  In almost all cases,
%   the changes reflect shifting from a vector input to a matrix input
%   where instead of individual values for each data point (trials), you 
%   have have vectors of functional data for each data "point" (trials). It
%   is possible I missed marking some of the changes, so if you find any,
%   let me know.
%
%   The other major change is the inclusion of a correction for sphericity
%   (see STEP 12) when you are dealing with a random factor - this is a bit 
%   of a hack solution, and could likely be improved, but it works for now.  
%   It calculates several corrections (Greenhouse-Geisser, Huynh-Feldt,
%   etc) but the remainder of the script uses GG - you'd have to change
%   these at a couple of points if you wanted a different correction
%   applied.  The calculation of the corrected dfs uses scripts from the
%   Mathworks code forum - see the appropriate m-files (e.g. epsGG.m) for
%   who deserves the credit for getting them working.
%   One further note about the sphericity correction - it is currently
%   applied to each point in the function separately.  Given the functional
%   nature of the data, there is perhaps some way you could apply some
%   overall functional correction, but for now, the correction applied at
%   each point appears to work.
%
%   FANOVA CHANGES TO INPUT AND OUTPUT
%  
%   All inputs are identical except that the Y argument (data) can be in
%   matrix (function) rather than vector (discrete) form.  When in matrix
%   form each ROW represents the functional data from ONE trial (or
%   measured function).  Each COLUMN represents a single data point from
%   that function.  So row 1 would be trial 1 and column 10 would be the 
%   10th data point in that function.  If you had 500 trials of data, and 
%   each trial was represented by 100 data points, Y would be a 500 x 100 
%   matrix.
%
%   All outputs are identical except that instead of returning a single
%   F-stat and corresponding p-value, when passed a functional Y, fanovan
%   returns a functional F-stat and p-value.  That is, you will get a
%   vector of F-stats and p-values, one for each COLUMN in your original Y
%   data matrix.  Depending on what your functions represent, this gives
%   you a measure of how significance changes across time (or space, or
%   whatever your function was representing).  Fanovan will also return a
%   corrected p-value in cases where you are dealing with a repeated
%   measures design (i.e. one factor is random, see note above).
%
%   I have currently removed the outputting of an ANOVA table since this
%   doesn't really apply for functional data.  In theory you can use
%   fanovan to run ANOVAs on discrete data, but I would still used anovan
%   in those cases to preserve this and other functionality (e.g. the
%   ability to use multCompare [though this does not allow for random /
%   repeated measures multiple comparissons])
...
%FANOVA edit
%Remove requirement of y as vector to allow for FDA data
%also remove conditional transpose since y is never a vector
% if (numel(y)~=length(y))
...
%FANOVA edit
%need to make statgetargs function non-private (I just cut and paste the
%m-file from xxxx to a directory on my path)
if newstyle
...
%FANOVA edit
%change n to reflect number of rows
n = size(y,1);
...
%FANOVA edit
%expand so mu matches dims with the functional y
mu = repmat(mu,n,1);
y = y - mu;                   % two passes to improve accuracy
mu2 = mean(y);

%FANOVA edit
%expand so mu2 matches dims with functional y
mu2 = repmat(mu2,n,1);
mu = mu + mu2;
y = y - mu2;
sst = sum(y.^2);
...
%change to reflect proper dims

%FANOVA edit
%expand so ssw matches dims with functional y
...
%FANOVA edit
%expand so ssw and dfw matches dims with functional y
ssw(k,:) = repmat(ssx,size(ssw,1),1);        % for full model we already know the results
dfw(k,:) = repmat(dfx,size(ssw,1),size(ssw,2));
...
%FANOVA edit
%change to match functional dims
fstat = repmat(Inf, size(msterm));

%FANOVA edit
%change and use the first data point of the function to tell you what terms
%to use for fstat
t1 = (msdenom(:,1)>0);
t2 = (msterm(:,1)>0);

%FANOVA edit
%change so fstat out is vector not single value
...
%FANOVA edit
% change pval dimensions as in step 10 (which was for FStat)
pval = repmat(NaN, size(fstat));
t = (dfdenom(:,1)>0 & dfterm(:,1)>0);
pval(t,:) = 1 - fcdf(fstat(t,:), dfterm(t,:), dfdenom(t,:));
p = pval;

%FANOVA edit
%adding in a correction for sphericity - this is very likely able to be
%done earlier and more elegantly, but this is my solution for now
%DO ONLY if there are more than 2 levels of the factor, and only if there
%is a random factor
%STEP 12
%for each time point (functional point)
if ~isempty(randomterm)
    for i = 1:size(pval,2)
        [curEpsHF curEpsList curEpsGG]=GenCalcHFEps(y(:,i),[],cell2mat(group(1:end-1)'),group{end});
        EpsHF(:,i) = curEpsHF;
        EpsLis{i} = curEpsList;
        EpsGG(:,i) = curEpsGG;
    end
    ...
    %redo p calculation with corrected df values
    corrPval = pval;
    %get only the non-rdm factor terms (which should match with the output size
    %of the Eps calc above)
    nonRdm = find(termlist(:,end)==0);
    %calculating corrected p (note adjustment to df here using GG)
    corrPval(nonRdm,:) = 1 - fcdf(fstat(nonRdm,:), dfterm(nonRdm,:).*EpsGG, dfdenom(nonRdm,:).*EpsGG);
    corrP = corrPval;
end
    
%FANOVA edit
%Removed following code as a traditional ANOVA table is no longer useful
...
function fitsubmodels
   for j=length(sindices):-1:1
      ...
      if doems && j>nterms
       ...
       %FANOVA edit
       %I was getting strange errors when k was empty, says there is a matrix size
       %mismatch, even though you are using an empty index - just ignore in
       %this case
       if ~(size(k) == [0 0])      
           ssw(k) = ssx0;
           dfw(k) = dfx0;
       ...
       %FANOVA edit - again do the check for size of k, then make sswo and
       %dfwo and dfboth fit to functional dims
       if ~(size(k) == [0 0])
           sswo(k,:) = ssx0';
           dfwo(k,:) = repmat(dfx0,size(k,1),size(k,2));
       end

       dfboth(nterms+k,:) = zeros(size(k,1),size(k,2));
   end
end
...
function stats = makestats(stats)
   ...
   if doems
      ...
      %FANOVA edit
      %if using random effects, add in the corrected df info - note, again
      %I am using gg here
      %note, I am also outputting the functional version of msterm, since
      %in the regular anovan this information was available in the anova
      %table, which no longer works for the fanova.
      stats.msterm = msterm;
      stats.corrDfNum = dfterm(nonRdm,:).*EpsGG;
      stats.corrDfDenom = dfdenom(nonRdm,:).*EpsGG;
   else
      stats.ems = [];
      stats.denom = [];
      stats.dfdenom = [];
      ...
      stats.rtnames = [];
      %FANOVA edit
      %set them as empty in this case
      stats.msterm = [];
      stats.corrDfNum = [];
      stats.corrDfDenom = [];
   end
...
%FANOVA edit
%to allow this to be functional, change to matrix instead of vector
...
%FANOVA edit
%change to apply to each element/data point in function since norm does not
%do the right thing with a matrix, manually do the norm calc (see help
%norm) and apply to each element of y2
%ssx = norm(y2)^2;        % sum of squares explained by fit
ssx = sum(abs(y2).*abs(y2)).^1/2;
...
if (nargout >= 3)
   ...
   if ~isempty(t)
       %FANOVA edit
       %change to reflect functional dims
      t(1,:) = t(1,:) + mu(1,:);            % undo mean adjustment before storing
   end
   stats.coeffs = t;
...
%FANOVA edit
%change next 3 lines to reflect functional dims
nterms = size(msTerm,1);
msDenom = repmat(NaN,nterms,size(msTerm,2));
dfDenom = repmat(NaN,nterms,size(msTerm,2));
...
%FANOVA edit
%change next three lines to allow for functional
msTerm = [msTerm; msErr];
dfTerm = [dfTerm; repmat(dfErr,1,length(dfTerm))];
...
for j=1:nterms
   ...
   if length(nonzeroterms) == 1
      ...
      %FANOVA edit
      %change next 2 lines to reflect functional dims
