...
function [] = plotMultiHeadAngleHeatmap(traj_names, subplot_p, p)
    ...
    for iTraj = 1:length(traj_names)
        ...
        for iSub = good_subs
            ...
            data_con = [data_con; reshape(angles_con_mat, p.NORM_FRAMES * num_trials_con, 1)];
            data_incon = [data_incon; reshape(angles_incon_mat, p.NORM_FRAMES * num_trials_incon, 1)];
        end
        ...
        hold on;
        make_heat_map(data_con, 'con', subs_avg, p);

        % Plot incongruent data.
        subplot(subplot_p(2,1), subplot_p(2,2), subplot_p(2,3));
        hold on;
        make_heat_map(data_incon, 'incon', subs_avg, p);
    end
end

% Builds data for the x axis and then plots a colored histogram.
% cond - condition, 'con'/'incon'.
function [] = make_heat_map(data, cond, subs_avg, p)
    n_bins = [p.NORM_FRAMES, 100]; % in each axis (X, Y) of the histogram (heatmap).
