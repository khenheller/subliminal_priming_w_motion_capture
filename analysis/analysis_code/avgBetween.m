...
function [subs_avg] = avgBetween(traj_name, p)
    ...
    subs_avg.traj.same_left  = subs_avg.traj.same_left  / length(good_subs);
    subs_avg.traj.same_right = subs_avg.traj.same_right / length(good_subs);
    subs_avg.traj.diff_left  = subs_avg.traj.diff_left  / length(good_subs);
    subs_avg.traj.diff_right = subs_avg.traj.diff_right / length(good_subs);
    subs_avg.rt.same_left  = subs_avg.rt.same_left  / length(good_subs);
    subs_avg.rt.same_right = subs_avg.rt.same_right / length(good_subs);
    subs_avg.rt.diff_left  = subs_avg.rt.diff_left  / length(good_subs);
    subs_avg.rt.diff_right = subs_avg.rt.diff_right / length(good_subs);
    subs_avg.react.same_left  = subs_avg.react.same_left  / length(good_subs);
    subs_avg.react.same_right = subs_avg.react.same_right / length(good_subs);
    subs_avg.react.diff_left  = subs_avg.react.diff_left  / length(good_subs);
    subs_avg.react.diff_right = subs_avg.react.diff_right / length(good_subs);
    subs_avg.mt.same_left  = subs_avg.mt.same_left  / length(good_subs);
    subs_avg.mt.same_right = subs_avg.mt.same_right / length(good_subs);
    subs_avg.mt.diff_left  = subs_avg.mt.diff_left  / length(good_subs);
    subs_avg.mt.diff_right = subs_avg.mt.diff_right / length(good_subs);
    subs_avg.fc_prime.same = subs_avg.fc_prime.same / length(good_subs);
    subs_avg.fc_prime.diff = subs_avg.fc_prime.diff / length(good_subs);
    subs_avg.pas.same = subs_avg.pas.same / length(good_subs);
    subs_avg.pas.diff = subs_avg.pas.diff / length(good_subs);
    subs_avg.mad.same_left  = subs_avg.mad.same_left  / length(good_subs);
    subs_avg.mad.same_right = subs_avg.mad.same_right / length(good_subs);
    subs_avg.mad.diff_left  = subs_avg.mad.diff_left  / length(good_subs);
    subs_avg.mad.diff_right = subs_avg.mad.diff_right / length(good_subs);
    subs_avg.mad_p.same_left  = subs_avg.mad_p.same_left  / length(good_subs);
    subs_avg.mad_p.same_right = subs_avg.mad_p.same_right / length(good_subs);
    subs_avg.mad_p.diff_left  = subs_avg.mad_p.diff_left  / length(good_subs);
    subs_avg.mad_p.diff_right = subs_avg.mad_p.diff_right / length(good_subs);
    subs_avg.reach_area.same = mean(reach_area.same);
    subs_avg.reach_area.diff = mean(reach_area.diff);
    subs_avg.x_std.same_left  = subs_avg.x_std.same_left  / length(good_subs);
    subs_avg.x_std.same_right = subs_avg.x_std.same_right / length(good_subs);
    subs_avg.x_std.diff_left  = subs_avg.x_std.diff_left  / length(good_subs);
    subs_avg.x_std.diff_right = subs_avg.x_std.diff_right / length(good_subs);
    subs_avg.x_avg_std.same_left  = subs_avg.x_avg_std.same_left  / length(good_subs);
    subs_avg.x_avg_std.same_right = subs_avg.x_avg_std.same_right / length(good_subs);
    subs_avg.x_avg_std.diff_left  = subs_avg.x_avg_std.diff_left  / length(good_subs);
    subs_avg.x_avg_std.diff_right = subs_avg.x_avg_std.diff_right / length(good_subs);
end