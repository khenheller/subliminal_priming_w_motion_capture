...
function [] = plotAllTrajs(iSub, traj_names, subplot_p, plt_p, p)
    ...
    for iTraj = 1:length(traj_names)
        ...
        hold on;
        plot(r_single.trajs.con_left(:,:,1), r_single.trajs.con_left(:,:,3) * 100, 'Color',[plt_p.con_col plt_p.f_alpha]);
        plot(r_single.trajs.con_right(:,:,1), r_single.trajs.con_right(:,:,3) * 100, 'Color',[plt_p.con_col plt_p.f_alpha]);

        % Plot averages.
        plot(r_avg.traj.con_left(:,1), r_avg.traj.con_left(:,3)* 100, plt_p.con_avg_col, 'LineWidth',plt_p.avg_plot_width);
        plot(r_avg.traj.con_right(:,1), r_avg.traj.con_right(:,3)* 100, plt_p.con_avg_col, 'LineWidth',plt_p.avg_plot_width);
        ...
        xlabel('X');
        xlim([-0.12, 0.12]);
        xticks(plt_p.percent_path_ticks);
        ylabel(y_label);
        ylim([0, 100]);
        ...
        hold on;
        plot(r_single.trajs.incon_left(:,:,1), r_single.trajs.incon_left(:,:,3) * 100, 'Color',[plt_p.incon_col plt_p.f_alpha]);
        plot(r_single.trajs.incon_right(:,:,1), r_single.trajs.incon_right(:,:,3) * 100, 'Color',[plt_p.incon_col plt_p.f_alpha]);

        % Plot averages.
        plot(r_avg.traj.incon_left(:,1), r_avg.traj.incon_left(:,3)* 100, plt_p.incon_avg_col, 'LineWidth',plt_p.avg_plot_width);
        plot(r_avg.traj.incon_right(:,1), r_avg.traj.incon_right(:,3)* 100, plt_p.incon_avg_col, 'LineWidth',plt_p.avg_plot_width);
